...
The controlled data-unavailable/SMS-available success path, selected-SIM loss (D), and restoration of the same pending attempt at a no-send pre-claim boundary (G) are accepted. The later isolated Redmi `UNKNOWN_OUTCOME` migration and no-send state-machine acceptance is recorded at the end of this document. Remaining device/carrier cases include permission denial, other SMS-unavailable conditions, both transports unavailable, dual-SIM ambiguity beyond explicit selected-SIM loss, low-battery behavior, actual WorkManager framework scheduling, live delayed/missing/duplicate or ambiguous callbacks, and carrier/provider failure outcomes. No-send callback simulation is not carrier delivery. A live foreground-service restart while the selected SIM was disabled was not exercised in D.
...
1. The remaining physical SMS/telephony failure matrix, including permission denial, other SMS-unavailable conditions, both transports unavailable, dual-SIM ambiguity, low battery, actual WorkManager scheduling, live callback and carrier/provider failure outcomes. Selected-SIM loss, no-send restoration, and isolated no-send `UNKNOWN_OUTCOME` behavior are accepted separately below; they do not prove live carrier callback or delivery behavior.
...
## Durable unknown SMS handoff outcome — isolated Redmi acceptance, 2026-09-26

### Corrected transport contract

Previously, an absent sent callback after the 15-minute uncertainty timeout changed `HANDOFF_IN_PROGRESS` to `RETRY_PENDING`, allowing an automatic resend without a finite send-count limit. Room v9 instead persists `UNKNOWN_OUTCOME`: absence of a callback is neither confirmed failure nor success, creates no retry work, and cannot make the same attempt sendable automatically. Recovery and Journey completion preserve this ambiguous transport history rather than calling it `SUPERSEDED`. A matching late callback can reconcile the same handoff generation to `HANDED_OFF`, confirmed-failure `RETRY_PENDING` if budget remains, or `PERMANENT_FAILURE`; stale and duplicate callbacks are idempotent. `HANDED_OFF` still means Android reported successful handoff, not delivery or cloud freshness.

The existing durable `transportAttemptCount` is incremented atomically before handoff, so it is a conservative upper bound on `SmsManager` invocations. Candidate selection and claim SQL allow at most **two claims per logical fallback attempt**: the initial claim and at most one automatic retry, only after an explicit retryable sent callback. A second confirmed retryable failure records `PERMANENT_FAILURE` / `RETRY_EXHAUSTED`; no third claim is allowed. Missing callback never authorizes retry, including after restart or repeated worker dispatch. A later *new* logical fallback requires independently newer telemetry, an explicit sparse trigger, the minimum interval, and rate-window capacity; it receives a new envelope sequence without changing the ambiguous old attempt or its exact JC1.

### Isolated v8-to-v9 migration and no-send device evidence

- The Redmi migration fixture used `unknown-outcome-v8-v9-test.db` in a separate `com.journeycontinuity.app.m6isolation` application sandbox/UID, not the installed production package or its `journey-continuity.db`. The isolated runner substituted a plain `Application`; the original production APK path was unchanged. Isolated test packages and database fixtures were removed afterward. Production DB file hashes changed while the production app was active, so byte-for-byte stability is not claimed; package/UID and fixture-path isolation establish that the migration fixture did not access production Room rows.
- A v8 timeout-created `RETRY_PENDING` row with `UNKNOWN_OUTCOME` provenance became `UNKNOWN_OUTCOME` and ceased being a retry candidate. A confirmed-failure `RETRY_PENDING` row at count `1` remained unchanged and eligible for exactly its final permitted claim. A confirmed-failure row at count `2` became `PERMANENT_FAILURE` / `RETRY_EXHAUSTED`. `HANDED_OFF`, `ALLOCATED`, `SUPERSEDED`, and existing permanent-failure rows remained unchanged. Immutable JC1, nonce, envelope-sequence, and digest fingerprints and the exact transport-attempt counts survived migration; unchanged historical rows retained full-row fingerprints.
- The test-only gateway accepted a canonical synthetic JC1 at the handoff boundary but did not call `SmsManager`. With no sent callback, an accelerated test timeout changed `HANDOFF_IN_PROGRESS` to `UNKNOWN_OUTCOME`. Repeated uncertainty and retry dispatches produced no second handoff, new generation, or new logical envelope. The production timeout remains 15 minutes; the device test shortened only its injected clock/window.
- Separate instrumentation invocations closed/reopened on-disk Room state and recreated the instrumentation process. `UNKNOWN_OUTCOME`, count `1`, generation `1`, payload/digest, and lack of an automatic resend candidate persisted. A late matching success produced one `HANDED_OFF` result; duplicate success and stale-generation callbacks did not rewrite it. A late explicit retryable failure with count `1` produced `RETRY_PENDING` and one further claim. A second explicit retryable failure exhausted the budget; a second claim with no callback instead remained `UNKNOWN_OUTCOME` and could not make a third claim. Late explicit permanent failure became `PERMANENT_FAILURE`.
- Full internet recovery was asserted to reach `HEALTHY` without superseding `UNKNOWN_OUTCOME`; subsequent Journey completion likewise preserved it, and a matching late callback could still reconcile it. Ordinary ticks and telemetry did not create another attempt. A separate sparse trigger with newer telemetry and elapsed interval was blocked by a full rate window; a later eligible window allocated a new observation/sequence while the old ambiguous attempt stayed unchanged.

Six distinct unknown-outcome instrumentation tests were executed individually and the 12-test fallback-ledger Room class passed on the isolated Redmi. Focused coordinator/policy JVM tests and the full 147-test `testDebugUnitTest` suite passed; `assembleDebug`, `assembleDebugAndroidTest`, and `lintDebug` passed with 0 lint errors. `git diff --check` and the sensitive-value scan passed. **Zero real carrier SMS sends occurred.** No hosted infrastructure, AWS, or provider configuration changed.

### Exact acceptance limits and remaining matrix

The worker's extracted production dispatch path was exercised with a fake coordinator; Android **WorkManager framework scheduling itself was not directly exercised** and is not claimed accepted. One later test-ordering-only change compiled but the complete unknown-outcome class was not rerun on-device after HyperOS blocked reinstall; the six individual runtime assertions had already passed. No-send callback simulation does not establish live carrier sent-callback reliability, recipient delivery, provider receipt, or carrier-to-provider retry/latency.

Remaining device/telephony acceptance: permission denial; other SMS-unavailable conditions; both transports unavailable; dual-SIM ambiguity beyond accepted selected-SIM loss; low battery; live-service restart under selected-SIM loss; actual WorkManager scheduling; live delayed/missing/duplicate/ambiguous callback and carrier/provider failure behavior. Trusted-contact notification receipt and failure handling remain separate. Production-grade provider authentication and real carrier-to-provider delivery, retry, and latency remain unproven. Milestone 6 remains **IN PROGRESS**.
