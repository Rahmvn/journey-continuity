...
function aad(
  ...
  return new TextEncoder().encode(
    `journey-continuity-fallback-key|v1|${ownerId}|${installationIdentifier}|${keyId}|${version}`,
  );
}
...
async function importKek(
  rawKek: Uint8Array,
  usage: KeyUsage[],
): Promise<CryptoKey> {
  if (rawKek.length !== 32) {
    throw new Error("Fallback KEK must be exactly 32 bytes");
  }
...
export async function encryptFallbackMasterKey(
  ...
  if (plaintext.length !== MASTER_KEY_BYTES) {
    throw new Error("Fallback master key must be 32 bytes");
  }
    ...
    await crypto.subtle.encrypt(
      {
        ...
        additionalData: buffer(
          aad(ownerId, installationIdentifier, keyId, encryptionVersion),
        ),
        tagLength: 128,
      },
      key,
...
export async function decryptFallbackMasterKey(
  ...
  if (
    encrypted.iv.length !== GCM_IV_BYTES ||
    encrypted.ciphertext.length !== MASTER_KEY_BYTES + GCM_TAG_BYTES
  ) {
...
export function parseKekRing(
  ...
  if (!Number.isSafeInteger(activeVersion) || activeVersion <= 0) {
    throw new Error("Invalid active KEK version");
  }
  ...
  if (!keys.has(activeVersion)) {
    throw new Error("Active fallback KEK is unavailable");
  }
